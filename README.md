...
- Added data export and import buttons for individual and all scripts.
  - The settings are exported in .json format.
  - Known issue: Some scripts like Marker's User Blocker use custom ui elements that will only update on page refresh.
...
*entryConfig* is an object that contains:
 ...
 - <a name="entry_type" href="#entry_type">`type`</a>: A `String` of either `checkbox`, `text`, `textarea`, `number`, `radio`, or `dropdown`.
   ...
   | `textarea` | String           | `<textarea>` with `type="text"`     |
